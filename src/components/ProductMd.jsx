        ...
        <section className='max-padd-container flex flex-col gap-8 xl:flex-row bg-primary py-4'>
            ...
            <div className="flex gap-x-2 xl:flex-1 py-5">
                <div className="flexCenter flex-col gap-[7px] flex-wrap">
                    <img src={`${url}/images/${product.image}`} alt='productImg' className='max-h-[88px] rounded-lg bg-gray-10' />
                    <div className='max-h-[377px] w-auto flex'>
                        <img src={`${url}/images/${product.image}`} alt='bigProductImg' className='max-h-[377px] rounded-lg bg-gray-10' />
                    </div>
                </div>
                {/* On your Right */}
                <div className='flex-col flex xl:flex-[1.5] bg-white px-6 py-2 rounded-xl'>
                    ...
                    <div className="flex flex-col sm:flex-row gap-x-10 gap-y-3 my-4">
                        <div>
                            ...
                            <div className='flex gap-3 my-3'>
                                <div onClick={() => setSelectedColor('red')} className={`h-10 w-10 flexCenter cursor-pointer rounded-sm bg-secondaryRed ${selectedColor === 'red' ? 'border border-black' : ''}`}></div>
                                <div onClick={() => setSelectedColor('yellow')} className={`h-10 w-10 flexCenter cursor-pointer rounded-sm bg-secondaryYellow ${selectedColor === 'yellow' ? 'border border-black' : ''}`}></div>
                                <div onClick={() => setSelectedColor('blue')} className={`h-10 w-10 flexCenter cursor-pointer rounded-sm bg-secondaryBlue ${selectedColor === 'blue' ? 'border border-black' : ''}`}></div>
                                <div onClick={() => setSelectedColor('green')} className={`h-10 w-10 flexCenter cursor-pointer rounded-sm bg-secondaryGreen ${selectedColor === 'green' ? 'border border-black' : ''}`}></div>
                            </div>
                        </div>
                        <div>
                            ...
                            <div className="flex gap-3 my-3">
                                {['S', 'M', 'L', 'XL'].map(size => (
                                    <div 
                                        ...
                                        className={`border-[1.5px] h-10 w-10 flexCenter cursor-pointer rounded-sm ${selectedSize === size ? 'border-black' : 'border-slate-900/15'}`}>
                                        {size}
                                    </div>
                                ))}
                            </div>
                    ...
                    <div className="flex gap-4 mb-8 max-w-[555px] flex-wrap">
                        ...
                        {!cartItems[product._id] ? (
                            <FaPlus
                                onClick={() => addToCart(product._id, selectedColor, selectedSize)}
